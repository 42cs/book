...
var createAudioTourHTML = function (divid, code, bnum, audio_text) {
    ...
    html_string += "<input type='image' src='../static/first.png' width='25' id='first_audio' name='first_audio' title='Play first audio in tour' alt='Play first audio in tour' disabled/>" + "<input type='image' src='../static/prev.png' width='25' id='prev_audio' name='prev_audio' title='Play previous audio in tour' alt='Play previous audio in tour' disabled/>" + "<input type='image' src='../static/pause.png' width='25' id='pause_audio' name='pause_audio' title='Pause current audio' alt='Pause current audio' disabled/><input type='image' src='../static/next.png' width ='25' id='next_audio' name='next_audio' title='Play next audio in tour' alt='Play next audio in tour' disabled/><input type='image' src='../static/last.png' width ='25' id='last_audio' name='last_audio' title='Play last audio in tour' alt='Play last audio in tour' disabled/><br/>";
    for (var i = 0; i < audio_type.length - 1; i++) {
...
var tour = function (divid, audio_type, bcount) {
    ...
    for (i = 1; i < max - 1; i++) {
        ...
        str += "<audio id=" + akey + " preload='auto' ><source src='../static/audio/" + akey +
            ".wav' type='audio/wav'><source src='../static/audio/" +
...
var pauseAndPlayAudio = function () {
    ...
    if (elem.paused) {
        ...
        document.getElementById("pause_audio").src = "../static/pause.png";
    ...
    // if audio was playing pause it
        ...
        document.getElementById("pause_audio").src = "../static/play.png";
